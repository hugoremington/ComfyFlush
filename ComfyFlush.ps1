...
try
{
    while($True)
    {
        ...
        # Inject the VENV into the session PATH
        # This allows us to call "python" directly as a native command
        ...
        # Start the process
        # Because 'python' is now in the PATH, we no longer need the long, problematic path variable.
        # We use 'cmd /c' only to handle the log redirection (>>).
